...
* **Inversión de Dependencias (DIP):** El código depende de abstracciones como `InputReader` y `SolutionPrinter`, lo que permite cambiar la fuente de datos o la salida  sin modificar la lógica.
* **Abierto-Cerrado (OCP):** El diseño es flexible. Gracias a que la longitud de búsqueda (`targetLength`) se pasa como parámetro al Factory Method `withDigitLength`, el sistema puede adaptarse para buscar secuencias de disitinto numero dígitos sin alterar el código interno de la clase `CalibrationController`.
